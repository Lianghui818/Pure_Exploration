...
class simulateOnlineData(object):
	...
	def runAlgorithms(self, algorithms):
		...
		for num_agents in agent_num:
			n_users = num_agents
			sc_agent[num_agents] = copy.deepcopy(SampleComList_D_Hom)
			cc_agent[num_agents] = copy.deepcopy(CommCostList_D)
		...
		if (self.plot==True): # only plot
			...
			file_name = ['SampleComplex_dataset2_05_17_08_09', 'SampleComplex_dataset2_10_14_19_39','SampleComplex_dataset2_10_14_19_39',
			 'SampleComplex_dataset2_10_13_21_19', 'SampleComplex_dataset2_10_13_21_34']
			suffix = '.csv'
			...
			axa[0].fill_between(sx, mean_L-std_L, mean_L+std_L, color='blue', alpha=0.2)
			axa[0].plot(sx, syuh, marker='o', linestyle='dotted', label='LinGapE-Single')
			axa[0].fill_between(sx, mean_U-std_U, mean_U+std_U, color='red', alpha=0.2)
			...
			cyu = []
			cyd = []
			path1 = 'C:/Users/Administrator/Desktop/Async-LinUCB-master/SimLinear/'
			file_name1 = ['AccCommCost_dataset2_05_17_08_09', 'AccCommCost_dataset2_10_14_19_53','AccCommCost_dataset2_10_13_21_34',
			 'AccCommCost_dataset2_10_14_19_34', 'AccCommCost_dataset2_10_13_21_19']
			suffix1 = '.csv'
			
			values_L1 = [[] for _ in range(5)]  
			values_U1 = [[] for _ in range(5)]
			values_D1 = [[] for _ in range(5)]
			
			for file in file_name1:
				full_path1 = path1+file+suffix1
				df1 = pd.read_csv(full_path1)
				for i in range(5):
					values_L1[i].append(df1.iloc[i , 0])
					values_U1[i].append(df1.iloc[i + 5, 0])
					values_D1[i].append(df1.iloc[i + 10, 0]) 
					           
			values_L1 = np.array(values_L1).astype(float)
			values_U1 = np.array(values_U1).astype(float)
			values_D1 = np.array(values_D1).astype(float)
			
			mean_L1 = np.mean(values_L1, axis=1)
			std_L1 = np.std(values_L1, axis=1)
			mean_U1 = np.mean(values_U1, axis=1)
			std_U1 = np.std(values_U1, axis=1)
			mean_D1 = np.mean(values_D1, axis=1)
			std_D1 = np.std(values_D1, axis=1)
			print("=====Comm Cost=====")
			for alg_name in algorithms.keys():
				if alg_name[17] == 'L' and alg_name[13:16] == 'Hom':
			...
			axa[1].fill_between(cx, mean_L1-std_L1, mean_L1+std_L1, color='blue', alpha=0.2)
			...
			axa[1].fill_between(sx, mean_D1-std_D1, mean_D1+std_D1, color='red', alpha=0.2)
			...
			fig, ax = plt.subplot()
			for num_agents in agent_num:
				ax.plot(sc_agent[num_agents].keys(), sc_agent[num_agents].values(), marker='v', linestyle='dotted', label=f'{num_agents} Agents')
				ax.plot(cc_agent[num_agents].keys(), cc_agent[num_agents].values(), marker='o', linestyle='dotted', label=f'{num_agents} Agents')
...
if __name__ == '__main__':
	...
	# 	algorithms['gap=.1_data0_Sin_L'] = LinGapE_mult(dimension=5, epsilon=epsilon, 
	# 							delta= delta, NoiseScale=NoiseScale, K = 5,
	# 							dataset=0, case='linear', gap=1)
	# 	algorithms['gap=.2_data0_Sin_L'] = LinGapE_mult(dimension=5, epsilon=epsilon, 
	# 							delta= delta, NoiseScale=NoiseScale, K = 5,
	# 							dataset=0, case='linear', gap=2)
	# 	algorithms['gap=.3_data0_Sin_L'] = LinGapE_mult(dimension=5, epsilon=epsilon, 
	# 							delta= delta, NoiseScale=NoiseScale, K = 5,
	# 							dataset=0, case='linear', gap=3)
	# 	algorithms['gap=.4_data0_Sin_L'] = LinGapE_mult(dimension=5, epsilon=epsilon, 
	# 							delta= delta, NoiseScale=NoiseScale, K = 5,
	# 							dataset=0, case='linear', gap=4)
	# 	algorithms['gap=.5_data0_Sin_L'] = LinGapE_mult(dimension=5, epsilon=epsilon, 
	# 							delta= delta, NoiseScale=NoiseScale, K = 5,
	# 							dataset=0, case='linear', gap=5)

			
		
	# 	algorithms['gap=.1_data0_Hom_D'] = DisALinPE(dimension=5, epsilon=epsilon, 
	# 							delta= delta, NoiseScale=NoiseScale, K = 5,
	# 							dataset=dataset, case='linear', gap=1)
	# 	algorithms['gap=.2_data0_Hom_D'] = DisALinPE(dimension=5, epsilon=epsilon, 
	# 							delta= delta, NoiseScale=NoiseScale, K = 5,
	# 							dataset=dataset, case='linear', gap=2)
	# 	algorithms['gap=.3_data0_Hom_D'] = DisALinPE(dimension=5, epsilon=epsilon, 
	# 							delta= delta, NoiseScale=NoiseScale, K = 5,
	# 							dataset=dataset, case='linear', gap=3)
	# 	algorithms['gap=.4_data0_Hom_D'] = DisALinPE(dimension=5, epsilon=epsilon, 
	# 							delta= delta, NoiseScale=NoiseScale, K = 5,
	# 							dataset=dataset, case='linear', gap=4)
	# 	algorithms['gap=.5_data0_Hom_D'] = DisALinPE(dimension=5, epsilon=epsilon, 
	# 							delta= delta, NoiseScale=NoiseScale, K = 5,
	# 							dataset=dataset, case='linear', gap=5)
		...
		# algorithms['gap=.1_data0_Hom_D'] = DisALinPE(dimension=context_dimension, epsilon=epsilon, 
		# 						delta= delta, NoiseScale=NoiseScale, K = n_articles,
